...
from bayesian_layers import VariationalBayesianLinear
...
class QNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(3136, 512),
            nn.ReLU(),
            VariationalBayesianLinear(512, env.single_action_space.n),
        )
...
if __name__ == "__main__":
   ...
   # target_network.load_state_dict(q_network.state_dict())
    ...
    for global_step in range(args.total_timesteps):
        ...
        actions = torch.tensor([np.random.choice(np.flatnonzero(q == q.max())) for q in q_values])
        #eps greedy actions
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    next_probabilities = torch.full(next_q_values.shape, 0, device=device) #deterministic
                    next_probabilities.scatter_(1, next_q_values.argmax(dim=1, keepdim=True), 1) #deterministic
                    expected_next_q = (next_q_values * next_probabilities).sum(dim=1) # can jut be set to max(q)
